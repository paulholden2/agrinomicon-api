defmodule AgrinomiconWeb.OrganizationControllerTest do
  ...
  describe "show organization" do
    ...
    test "renders organization when valid ID is passed", %{conn: conn, organization: organization} do
      conn = get(conn, ~p"/api/organizations/#{organization.id}")
      assert json_response(conn, 200)
    end

    test "renders 404 when invalid ID is passed", %{conn: conn} do
      conn = get(conn, ~p"/api/organizations/not_a_binary_id")
